...
// .wrangler/tmp/bundle-xfWaC9/checked-fetch.js
...
async function handleRegister(request, env, corsHeaders) {
  try {
    ...
    await env.DB.prepare("INSERT INTO users (id, email, password_hash, username, role) VALUES (?, ?, ?, ?, ?)").bind(
      userId,
      email,
      passwordHash,
      username,
...
__name(handleGetQuestions, "handleGetQuestions");
...
__name(handleGetQuestion, "handleGetQuestion");
...
__name(handleCreateQuestion, "handleCreateQuestion");
...
__name(handleCreateAnswer, "handleCreateAnswer");
...
__name(checkAdminAccess, "checkAdminAccess");
...
__name(handleGetTranslations, "handleGetTranslations");
...
async function handleAdminUpdateSettings(request, env, corsHeaders) {
  try {
    ...
    if (site_title) {
      statements.push(
        env.DB.prepare('INSERT OR REPLACE INTO site_settings (key, value, updated_at) VALUES (?, ?, datetime("now"))').bind("site_title", site_title)
      );
    }
    if (site_tagline) {
      statements.push(
        env.DB.prepare('INSERT OR REPLACE INTO site_settings (key, value, updated_at) VALUES (?, ?, datetime("now"))').bind("site_tagline", site_tagline)
      );
    }
    if (default_language && (default_language === "en" || default_language === "tr")) {
      statements.push(
        env.DB.prepare('INSERT OR REPLACE INTO site_settings (key, value, updated_at) VALUES (?, ?, datetime("now"))').bind("default_language", default_language)
      );
    }
...
__name(handleAdminUpdateSettings, "handleAdminUpdateSettings");
...
__name(handleAdminUpdateUserRole, "handleAdminUpdateUserRole");
...
__name(handleAdminToggleBan, "handleAdminToggleBan");
...
__name(handleAdminGetQuestions, "handleAdminGetQuestions");
...
__name(handleAdminGetAnswers, "handleAdminGetAnswers");
...
__name(handleAdminUpdateQuestion, "handleAdminUpdateQuestion");
...
__name(handleAdminUpdateAnswer, "handleAdminUpdateAnswer");
...
__name(handleAdminDeleteAnswer, "handleAdminDeleteAnswer");
async function handleVote(request, env, itemType, itemId, corsHeaders) {
  try {
    ...
    const existingVote = await env.DB.prepare("SELECT vote FROM votes WHERE user_id = ? AND item_type = ? AND item_id = ?").bind(userId, itemType, itemId).first();
    if (vote === 0) {
      if (existingVote) {
        await env.DB.prepare("DELETE FROM votes WHERE user_id = ? AND item_type = ? AND item_id = ?").bind(userId, itemType, itemId).run();
        await env.DB.prepare(`UPDATE ${tableName} SET votes = votes - ? WHERE id = ?`).bind(existingVote.vote, itemId).run();
      }
    } else if (existingVote) {
      const voteDiff = vote - existingVote.vote;
      await env.DB.prepare("UPDATE votes SET vote = ? WHERE user_id = ? AND item_type = ? AND item_id = ?").bind(vote, userId, itemType, itemId).run();
      await env.DB.prepare(`UPDATE ${tableName} SET votes = votes + ? WHERE id = ?`).bind(voteDiff, itemId).run();
    } else {
      await env.DB.prepare("INSERT INTO votes (user_id, item_type, item_id, vote) VALUES (?, ?, ?, ?)").bind(userId, itemType, itemId, vote).run();
      await env.DB.prepare(`UPDATE ${tableName} SET votes = votes + ? WHERE id = ?`).bind(vote, itemId).run();
    }
...
async function renderHomePage(env, page, corsHeaders) {
  try {
    ...
    if (questions.length === 0) {
      ...
      questions.forEach((q) => {
        const answerText = q.answer_count === 1 ? translations["misc.answer_singular"] || "answer" : translations["misc.answer_plural"] || "answers";
        questionsHTML += `
...
async function renderQuestionPage(env, questionId, corsHeaders) {
  try {
        ...
        answersHTML += `
          <div class="answer-detail">
            <div class="vote-section">
              ...
              <span class="vote-count" id="answer-${a.id}-votes">${a.votes || 0}</span>
    ...
    const bodyContent = `
      ...
      <div class="question-detail">
        ...
        <div class="content-body">
          ...
          <div class="answer-meta">
            ${question.username ? `<span>${translations["user.asked_by"] || "soran"} <a href="/u/${escapeHtml(question.username)}">@${escapeHtml(question.username)}</a></span>` : "<span>" + (translations["misc.anonymous"] || "Anonim") + "</span>"}
            <span>\u2022</span>
    ...
    return new Response(getBaseHTML(title, description, siteTitle, siteTagline, lang, bodyContent, translations), {
      headers: { ...corsHeaders, "Content-Type": "text/html; charset=utf-8" }
    });
  } catch (error) {
    return new Response(getBaseHTML("Q&A Platform", "Ask questions, get answers", "Q&A Platform", "Ask questions, get answers", "tr"), {
...
__name(renderQuestionPage, "renderQuestionPage");
function getBaseHTML(pageTitle, description, siteTitle, siteTagline, lang = "tr", bodyContent = "", translations = {}) {
      ...
      <div class="header-content">
        <div>
          <a href="/" class="header-link">
            <h1 id="siteTitle">${escapeHtml(siteTitle)}</h1>
          </a>
...
__name(getBaseHTML, "getBaseHTML");
...
h1 { font-size: 24px; font-weight: 600; margin: 0; color: #24292f; }
...
.breadcrumb { margin-bottom: 20px; padding-bottom: 16px; border-bottom: 1px solid #d0d7de; color: #57606a; font-size: 13px; }
...
@media (max-width: 768px) { header { padding: 12px 16px; } #app { width: 95%; padding: 16px; } .header-content { flex-direction: column; align-items: flex-start; gap: 12px; padding: 0 16px; } .modal { min-width: unset; width: 90%; padding: 24px; } .question-card { flex-direction: column; } }
...
var JS = `class App {
  ...
  async init() {
    try {
      ...
      this.app.innerHTML = '<div class="loading">' + (this.translations['error.load_app'] || 'Uygulama y\xFCklenemedi. L\xFCtfen yenileyin.') + '</div>';
    }
  }
  ...
  populateAnswerForm(questionId, placeholder) {
    if (this.currentUser) {
      placeholder.innerHTML = \`
        <div class="auth-form">
          ...
          <form id="answerForm">
            ...
            <button type="submit" class="submit-btn">\${this.t('button.post_answer', 'Cevap G\xF6nder')}</button>
          </form>
        </div>
      \`;
      ...
      placeholder.innerHTML = \`<div class="auth-link">\${this.t('message.login_to_answer', 'Cevap g\xF6ndermek i\xE7in giri\u015F yap\u0131n')}</div>\`;
    }
  }
  ...
  async showHome(page = 1) {
    ...
    window.app._hasNavigated = true;
    this.app.innerHTML = '<div class="loading">' + this.t('misc.loading_questions', 'Sorular y\xFCkleniyor...') + '</div>';
  ...
  async showQuestion(questionId) {
    this.app.innerHTML = '<div class="loading">' + this.t('misc.loading_question', 'Soru y\xFCkleniyor...') + '</div>';
    ...
    try {
      ...
      this.app.innerHTML = \`
        ...
        <div class="answers-section">
          <h3 class="answers-title">\${this.t('misc.answers', 'Answers')} (\${answers.length})</h3>
          ...
          \` : '<div class="no-answers">' + this.t('message.no_answers_long', 'Bu soru i\xE7in hen\xFCz cevap yok. \u0130lk cevaplayan siz olun!') + '</div>'}
        </div>
        ...
        \` : '<div class="auth-link">' + this.t('message.login_to_answer', 'Cevap g\xF6ndermek i\xE7in giri\u015F yap\u0131n') + '</div>'}
      \`;
      ...
      this.app.innerHTML = '<div class="loading">' + this.t('error.load_question', 'Soru y\xFCklenemedi') + ': ' + error.message + '</div>';
    }
  }
    ...
    this.app.innerHTML = \`
      ...
      <div class="admin-container">
        <div class="admin-header">
          <div>
            <h1 style="margin: 0;">\${this.t('admin.panel', 'Y\xF6netim Paneli')}</h1>
          ...
          <a href="/" class="admin-btn">\${this.t('button.back_site', '\u2190 Siteye D\xF6n')}</a>
        </div>
            ...
            <form id="settingsForm">
              ...
              <div class="form-group">
                ...
                <select id="defaultLanguage" style="width: 100%; padding: 12px; background: #fff; border: 1px solid #d0d7de; color: #24292f; font-size: 14px;">
                  <option value="tr" \${(settings.default_language || 'tr') === 'tr' ? 'selected' : ''}>T\xFCrk\xE7e (Turkish)</option>
        ...
        <div id="admin-users" class="admin-content">
          <div class="admin-card">
            <h3>\${this.t('admin.all_users', 'T\xFCm Kullan\u0131c\u0131lar')} (\${users.users.length})</h3>
            ...
            <table class="admin-table">
              <thead>
                <tr>
                  ...
                  <th>\${this.t('table.content', '\u0130\xE7erik \xD6nizleme')}</th>
                  <th>\${this.t('table.author', 'Yazar')}</th>
                  <th>\${this.t('table.votes', 'Oylar')}</th>
                  <th>\${this.t('table.date', 'Tarih')}</th>
                  <th>\${this.t('table.actions', '\u0130\u015Flemler')}</th>
    ...
    this.app.innerHTML = \`
      <div class="auth-form">
        ...
        <form id="passwordForm">
          ...
          <button type="submit" class="submit-btn">\${this.t('button.update_password', '\u015Eifreyi G\xFCncelle')}</button>
        </form>
      </div>
  ...
  async handleAskQuestion(e) {
    ...
    try {
      ...
      if (response.ok) {
        messageEl.innerHTML = '<div class="success-message">' + this.t('success.question_posted', 'Soru ba\u015Far\u0131yla g\xF6nderildi!') + '</div>';
        setTimeout(() => this.navigate(\`/q/\${data.questionId}\`), 1000);
      } else {
        messageEl.innerHTML = \`<div class="error-message">\${data.error}</div>\`;
      }
    } catch (error) {
      messageEl.innerHTML = '<div class="error-message">' + this.t('error.post_question', 'Soru g\xF6nderilemedi') + ': ' + error.message + '</div>';
    }
  }
  ...
  async handlePostAnswer(e, questionId) {
    ...
    try {
      ...
      if (response.ok) {
        messageEl.innerHTML = '<div class="success-message">' + this.t('success.answer_posted', 'Cevap ba\u015Far\u0131yla g\xF6nderildi!') + '</div>';
        setTimeout(() => this.showQuestion(questionId), 1000);
      } else {
        messageEl.innerHTML = \`<div class="error-message">\${data.error}</div>\`;
      }
    } catch (error) {
      messageEl.innerHTML = '<div class="error-message">' + this.t('error.post_answer', 'Cevap g\xF6nderilemedi') + ': ' + error.message + '</div>';
    }
  }
  ...
  getQualityBadge(votes) {
    if (votes >= 5) {
      return '<span style="background: linear-gradient(135deg, #27ae60, #2ecc71); color: white; padding: 0.375rem 0.875rem; border-radius: 16px; font-size: 0.8125rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 2px 4px rgba(39, 174, 96, 0.3);">' + this.t('badge.high_quality', 'Y\xFCksek Kalite') + '</span>';
    } else if (votes <= -3) {
      return '<span style="background: linear-gradient(135deg, #e74c3c, #c0392b); color: white; padding: 0.375rem 0.875rem; border-radius: 16px; font-size: 0.8125rem; font-weight: 600; text-transform: uppercase; letter-spacing: 0.5px; box-shadow: 0 2px 4px rgba(231, 76, 60, 0.3);">' + this.t('badge.low_quality', 'D\xFC\u015F\xFCk Kalite') + '</span>';
    }
  ...
  async vote(itemType, itemId, voteValue) {
    if (!this.currentUser) {
      alert(this.t('error.login_required', 'L\xFCtfen oy vermek i\xE7in giri\u015F yap\u0131n'));
      this.navigate('/login');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const voteElementId = itemType + '-' + itemId + '-votes';
        ...
        const badgeElementId = itemType + '-' + itemId + '-badge';
  ...
  async handleSaveSettings(e) {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        messageEl.innerHTML = '<div class="success-message">' + this.t('success.settings_saved', 'Ayarlar kaydedildi! Yeniden y\xFCkleniyor...') + '</div>';
  ...
  async toggleUserRole(userId, currentRole) {
    ...
    if (!confirm(this.t('confirm.change_role', 'Kullan\u0131c\u0131 rol\xFCn\xFC de\u011Fi\u015Ftir') + ': ' + newRole + '?')) return;
    ...
    try {
      ...
      alert(this.t('error.update_user_role', 'Kullan\u0131c\u0131 rol\xFC g\xFCncellenemedi') + ': ' + error.message);
    }
  }
  ...
  async toggleBan(userId, currentBanned) {
    ...
    try {
      ...
      alert(this.t('error.update_ban_status', 'Kullan\u0131c\u0131 ban durumu g\xFCncellenemedi') + ': ' + error.message);
    }
  }
  ...
  async saveEditQuestion(id, modal) {
    ...
    try {
      ...
      alert(this.t('error.update_question', 'Soru g\xFCncellenemedi') + ': ' + error.message);
    }
  }
  ...
  async saveEditAnswer(id, modal) {
    ...
    try {
      ...
      alert(this.t('error.update_answer', 'Cevap g\xFCncellenemedi') + ': ' + error.message);
    }
  }
...
// .wrangler/tmp/bundle-xfWaC9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-xfWaC9/middleware-loader.entry.ts
